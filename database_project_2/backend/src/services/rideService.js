...
exports.exitRideUsingCard = async (id, ID, endStation, endTime) => {
  try {
    ...
    if (exitRide.class == "BUSINESS") {
      await exitRide.update({
        end_station: endStation,
        end_time: endTime,
        price: 2 * JourneyPrice,
        status: "EXPIRED",
      });
...
exports.registerRideUsingPassenger = async (
  ID,
  StartStation,
  StartTime,
  type
...
const getPrice = async (startStation, endStation) => {
  try {
    const Journey = await Price.findOne({
      where: { start_station: startStation, end_station: endStation },
    });
